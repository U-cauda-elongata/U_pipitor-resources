...
in  [ Pipitor.Rule::{
      ...
      , exclude = Some Pipitor.Filter::{ title = rx.hash "今日のニコニコ生放送" }
      ...
      , topics = [
          ...
          Twitter 365469866, -- @hulu_japan Hulu Japan
    ...
    , Pipitor.Rule::{
      , filter = Some Pipitor.Filter::{ title = filter.basicMinusHanamaru }
      , outbox = [ Pipitor.Outbox.Twitter account.pipitor ]
      , topics = [
    ...
    , Pipitor.Rule::{
      , -- English
        filter = Some Pipitor.Filter::{ title = filter.basic }
      , exclude = Some Pipitor.Filter::{ title = filter.basicExclude }
